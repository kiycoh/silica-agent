...
def _vault_relative_folder(value: str) -> str:
    """A folder name the LLM produced, checked before it can name a directory.

    The taxonomy is generated by the model from vault note TITLES, so every
    folder string here is attacker-reachable through note content. It ends up
    in `os.path.join(vault, folder)` and in `DRIVER.move`, where an absolute
    path silently discards the vault root and a `..` segment walks out of it.
    Reject both at parse time: `enforce_scope_prefixes` below only PREPENDS a
    scope, and does nothing at all when the scope is empty.
    """
    norm = str(value or "").replace("\\", "/")
    if norm.startswith("/"):
        raise ValueError(f"folder must be vault-relative, got {value!r}")
    if any(part == ".." for part in norm.split("/")):
        raise ValueError(f"folder must not escape the vault, got {value!r}")
    return norm
...
class FolderRule(BaseModel):
    ...
    @classmethod
    def _folder_stays_in_the_vault(cls, v: str) -> str:
        return _vault_relative_folder(v)
...
class Taxonomy(BaseModel):
    ...
    @field_validator("uncategorized", "scope")
    @classmethod
    def _folders_stay_in_the_vault(cls, v: str) -> str:
        # `scope` too: it is PREPENDED to every rule folder below, so an
        # escaping scope would smuggle `..` back into a folder that validated
        # clean on its own.
        return _vault_relative_folder(v)
