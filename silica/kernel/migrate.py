...
migrate_adr_namespace moves docs/adr/ → .silica/adr/. Vault-relative
...
def migrate_adr_namespace(repo_root: Path | str) -> list[str]:
    """Move docs/adr/*.md → .silica/adr/. Returns moved vault-relative
    ...
    if moved:
        (old / "MOVED.md").write_text(
            "# Moved\n\nADRs now live in `.silica/adr/` (the Silica vault root).\n",
            encoding="utf-8",
        )
    return moved
